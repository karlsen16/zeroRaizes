...
MSS linear (int *A, int low, int high) {
  int i, temp = 0, temp_left = low;
  MSS ret = {low, low, INT_MIN};
...
int main () {
  ...
  MSS re1 = {0, t, INT_MIN};
  MSS re2 = {0, t, INT_MIN};
  double time_bf, time_rc, time_ln;
  while(1) {
    clock_t start, stop;
    ...
    for (i = 0; i < 1000000; i++)
      re2 = linear(A, 0, n);
    stop = clock();
    time_ln = ((double) (stop-start)) / CLOCKS_PER_SEC;
  ...
  printf("\nTempo gasto (LINEA): %21.10g ?s.\n", time_ln);
  printf("\n\nN = %d.\nSoma = %d.\nLeft = %d\nRight = %d.\n\n", n, res.sum, res.left+1, res.right);
  printf("\n\nN = %d.\nSoma = %d.\nLeft = %d\nRight = %d.\n\n", n, re1.sum, re1.left+1, re1.right);
  printf("\n\nN = %d.\nSoma = %d.\nLeft = %d\nRight = %d.\n\n", n, re2.sum, re2.left+1, re2.right);
